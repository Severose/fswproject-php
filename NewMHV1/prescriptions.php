...
function itemSelected($selectedOption)
{
  ...
  // Jaime link and connect to the database 
	...
	$query21 = "SELECT * FROM prescription WHERE resident_id =" . $selectedOption; 
	$result7 = mysql_query($query21 );
	if (!$result7) 
	{
		die ("Database query failed");
	}
	while ($row7 = mysql_fetch_array($result7))
	{
		...
		$x = new prescription(); 
		$x->number = $row7{'prescription_number'}; 
		$x->ordered = $row7{'date_ordered'}; 
		$x->received = $row7{'date_received'}; 
		$x->amount = $row7{'quantity'}; 
		$x->refill = $row7{'refill_date'}; 
		$x->medid = $row7{'medication_id'}; 
		
		
		array_push($GLOBALS["pre"], $x); 
